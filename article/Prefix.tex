...
We should be able to recognize a sequence of different tokens, so let's write a parser that is the composition two parsers applied in sequance. The expression \text{\ttfamily p~\char60{}\char38{}\char62{}~q} denotes a parser that fails if $p$ fails, or 
\begin{hscode}\SaveRestoreHook
\column{B}{@{}>{\hspre}l<{\hspost}@{}}%
\column{5}{@{}>{\hspre}l<{\hspost}@{}}%
